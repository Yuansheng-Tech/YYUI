...
export const YYList = (props: IListProps) => {
  const { hasBorder = false, data = [] } = props;
  return (
    <AtList hasBorder={hasBorder}>
      {data.map((val: IItemProps, key: number) => {
        const { title = '', handleClick = () => {}, onSwitchChange = () => {}, ...props } = val;
        return <AtListItem {...props} key={key} title={title} onClick={handleClick} onSwitchChange={onSwitchChange} />;
      })}
    </AtList>
  );
};
