...
import { useQuery } from '@ysyp/utils/useQuery';
import YYURL from '@ysyp/utils/url';
...
export const YYCartGood = (props: IProps & AtGridProps) => {
  ...
  return (<View
    ...
    <View
      className="yy-cart-good-item-left">
      {image && <Image
    ...
    <View
      className="yy-cart-good-item-desc">
      <Text
        className="yy-cart-good-item-title"
        style={{
          textAlign: 'left'
        }}>{value}</Text>
      {!!price && <View className="yy-cart-good-prices">
        <Text className='yy-cart-good-price'>¥{price}</Text>
        {!!originalPrice && (
          <Text className='yy-cart-good-original-price'>¥{originalPrice}</Text>
        )}
      </View>}
    </View>
  </View>)
